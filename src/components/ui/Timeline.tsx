      ...
      <div className="space-y-12">
        {entries.map((entry, i) => (
          <div key={i} className="relative">
            <div className="absolute -left-[33px] top-[6px] w-2 h-2 rounded-full bg-accent" />
            ...
            <ul className="space-y-2">
              {entry.bullets.map((bullet, j) => (
                <li key={j} className="text-muted text-sm leading-relaxed pl-4 relative before:absolute before:left-0 before:top-[0.55em] before:w-1 before:h-px before:bg-muted">
                  {bullet}
                </li>
              ))}
            </ul>
